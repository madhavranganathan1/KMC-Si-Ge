...
int kmc_optimization(void)
{
  ...
  T = gsl_rng_mt19937;
  gsl_rng_env_setup();
  gsl_rng_default_seed=abs(Seed3)+rank;
...
//perturbe
 if (Nomins >= 1*Maxnomintry ) { printf("failed - perturbing, In the kmc_optimization \n");
...
//perturbe
 if (Nomins >= 2*Maxnomintry ) { printf("failed - perturbing,  In the kmc_optimization\n");
  ss=ss+0.01,tg=tg+0.1,gt=gt+0.1;
         do {
...
//if(rank==0){printf("Kmc_optimiztion %lf\n",energyvalue);}
return 0;
}
